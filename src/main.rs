...
fn preprocess_file(
    ...
    loop { 
        // recursively check for directives until we run out (allows for
        // included files to have their own pp directives)
        let mut count: usize = 0;
        output = re.replace_all(&output, |cs: &regex::Captures| {
            count += 1;
            let strings = (&cs[1]).split_whitespace()
                .map(|s| s.to_string())
                .collect();
            pp(&pp_config, strings)
        }).into_owned();
        if count == 0 { break; }
    }
    ...
    loop { 
        ...
        output = re.replace_all(&output, |cs: &regex::Captures| {
            ...
            let strings = (&cs[1]).split_whitespace()
                .map(|s| s.to_string())
                .collect();
            pp_css(&pp_config, &mut pp_ctx, strings)
        }).into_owned();
        if count == 0 { break; }
    }
    format!(
